...
| T-05 | Oversized/repeated requests exhaust service | Denial of service | Content-length/file limits, timeout, sequential queue, in-memory throttle | Gateway body limits, distributed rate limits, queue backpressure, quotas |
...
| T-12 | In-memory rate key is spoofed or lost across instances | Denial of service | Basic prototype throttle only | Enforce trusted proxy headers and distributed gateway throttling |
